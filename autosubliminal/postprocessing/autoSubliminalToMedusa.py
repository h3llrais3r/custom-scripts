...
# It moves an episode and its subtitle from Auto-Subliminal to the Medusa post processing folder.
# CONFIGURATION: changes the values of AUTOSUBLIMINAL_PATH and MEDUSA_PATH to your paths.
...
# Paths
AUTOSUBLIMINAL_PATH = "//SERVER/medusa-autosubliminal/"
MEDUSA_PATH = "C:/Tools/Downloads/complete/medusa/"
...
NORM_MEDUSA_PATH = os.path.normcase(os.path.normpath(MEDUSA_PATH))

# Logging config (change to logging.DEBUG for debug info)
LOG_FILE = os.path.dirname(os.path.realpath(__file__)) + "/autoSubliminalToMedusa.log"
logging.basicConfig(filename=LOG_FILE, format='%(asctime)s - %(levelname)s - %(message)s', level=logging.INFO)
logger = logging.getLogger("autoSubliminalToMedusa")


def run():
    print ""
    print "#" * 30
    print "Medusa post processing folder: %s" % NORM_MEDUSA_PATH
    logger.info("----------------------------------------------")
    logger.info("Medusa post processing folder: %s" % NORM_MEDUSA_PATH)
...
def _move(episode_path, subtitle_path):
    try:
        destination = os.path.join(NORM_MEDUSA_PATH)
        episode = os.path.join(episode_path)
        shutil.move(episode, destination)
        print "Moved episode to the Medusa post processing folder"
        logger.info("Moved episode to the Medusa post processing folder")
        if subtitle_path:
            subtitle = os.path.join(subtitle_path)
            shutil.move(subtitle, destination)
            print "Moved subtitle to the Medusa post processing folder"
            logger.info("Moved subtitle to the Medusa post processing folder")
        return True
    except Exception, e:
        print "Exception: %s" % e
    ...
    try:
        ...
        if subtitles:
            ...
            for subtitle in subtitles:
                shutil.move(subtitle, destination)
                print "Moved additional found subtitle to the Medusa post processing folder: %s" % subtitle
                logger.info("Moved additional found subtitle to the Medusa post processing folder: %s" % subtitle)
